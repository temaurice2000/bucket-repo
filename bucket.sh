...
exist=$(/usr/local/bin/aws s3api head-bucket --bucket new-maurice-bucket-2022 2>&1 || true)
if [ -z "$exist" ]; then 
   echo "This bucket exists"
else
    echo "This bucket will be created with the name new-maurice-bucket-2022"
    /usr/local/bin/aws s3api create-bucket --bucket new-maurice-bucket-$Date --create-bucket-configuration LocationConstraint=us-west-2
fi 
